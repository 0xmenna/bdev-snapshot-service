...
struct dentry *singlefilefs_mount(struct file_system_type *, int, const char *,
                                  void *);
...
int singlefilefs_fill_super(struct super_block *sb, void *data, int silent) {
      ...
      sb->s_fs_info = NULL; // FS specific data (the magic number) already
                            // reported into the generic superblock
...
#error "Kernel versions older than 6.3.0 are no longer supported."
      ...
      // no inode from device is needed - the root of our file system is an in
      // memory object
...
struct dentry *singlefilefs_mount(struct file_system_type *fs_type, int flags,
      ...
      if (unlikely(IS_ERR(ret)))
            ...
            printk(
                "%s: singlefilefs is succesfully mounted on from device %s\n",
                MOD_NAME, dev_name);
...
static struct file_system_type onefilefs_type = {
    ...
    .fs_flags =
        FS_REQUIRES_DEV, // Indicate that the filesystem requires a device
};
...
static int singlefilefs_init(void) {
      ...
      if (likely(ret == 0))
            ...
            printk("%s: failed to register singlefilefs - error %d", MOD_NAME,
                   ret);
...
static void singlefilefs_exit(void) {
      ...
      if (likely(ret == 0))
            ...
            printk("%s: failed to unregister singlefilefs driver - error %d",
                   MOD_NAME, ret);
}
