...
ssize_t onefilefs_write(struct file *filp, const char __user *buf, size_t len,
      ...
      inode_lock(the_inode);
      file_size = the_inode->i_size;
      printk("%s: write operation called with len %ld - and offset %lld (the "
             "current file size is %lld)",
             MOD_NAME, len, *off, file_size);
      ...
      printk("%s: write operation must access block %d of the device", MOD_NAME,
             block_to_write);
      ...
      *off += (len - ret);
      ...
      if (the_inode->i_size < *off) {
            ...
            bh = (struct buffer_head *)sb_bread(
                filp->f_inode->i_sb, SINGLEFILEFS_INODES_BLOCK_NUMBER);
            if (!bh) {
                  inode_unlock(the_inode);
                  return -EIO;
            }
...
#error "Kernel versions older than 6.3.0 are no longer supported."
            ...
            // now we retrieve the file size via the FS specific inode, putting
            // it into the generic inode
            bh = (struct buffer_head *)sb_bread(
                sb, SINGLEFILEFS_INODES_BLOCK_NUMBER);
            if (!bh) {
                  iput(the_inode);
                  return ERR_PTR(-EIO);
            }
            ...
            printk("%s: file lookup operation - file size is %lld", MOD_NAME,
                   the_inode->i_size);
